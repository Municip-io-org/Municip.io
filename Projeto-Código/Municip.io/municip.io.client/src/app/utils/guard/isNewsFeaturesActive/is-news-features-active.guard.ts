...
export class IsNewsFeatureActive implements CanActivate {

  constructor(private appFeaturesService: AppFeaturesService, private router: Router) { }

  async canActivate(): Promise<boolean> {
    try {
      const isEnabled = await this.appFeaturesService.appFeatures.find(a => a.appFeatureCategory == "News")?.isEnabled;
      if (isEnabled) {
        return true;
      } else {
